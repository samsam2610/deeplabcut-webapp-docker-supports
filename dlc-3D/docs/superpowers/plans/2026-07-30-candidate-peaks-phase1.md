...
- Model: `hrnet_w48`, input 448×448, `num_heatmaps: 16`, `pad_width_divisor`/`pad_height_divisor` 32.
...
- `HeatmapPredictor.forward(stride, outputs)` receives `outputs["heatmap"]` of shape `(batch, joints, H, W)` and `outputs["locref"]`, applies a sigmoid when `apply_sigmoid`, then takes an argmax.
...
def main() -> int:
    ...
    for i in range(n):
        ...
        if not ok:
            ...
            break
        h, w = frame.shape[:2]
        scale = (net_w / w, net_h / h)
        img = cv2.resize(frame, (net_w, net_h))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
        t = torch.from_numpy(img).permute(2, 0, 1)[None].to(args.device)
        with torch.no_grad():
            out = model(t)
        hm = out["heatmap"] if isinstance(out, dict) else out[0]["heatmap"]
        hm = torch.sigmoid(hm)[0].cpu().numpy()      # (joints, H, W)
        stride = net_w / hm.shape[-1]
        for j in range(len(bodyparts)):
            cells, sc = pk.extract_peaks(hm[j], k=args.k,
                                         min_distance=args.min_distance)
            XY[i, j] = pk.heatmap_to_image(cells, stride, (0, 0), scale)
            SC[i, j] = sc
        if i % 50 == 0:
